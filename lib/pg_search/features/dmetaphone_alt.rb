module PgSearch
  module Features
    class DMetaphoneAlt
      def initialize(query, options, columns, model, normalizer)
        dmetaphone_alt_normalizer = Normalizer.new(normalizer)
        options = (options || {}).merge(:dictionary => 'simple')
        @tsearch = TSearch.new(query, options, columns, model, dmetaphone_alt_normalizer)
      end

      def conditions
        tsearch.conditions
      end

      def rank
        tsearch.rank
      end

      private

      attr_reader :tsearch

      # Decorates a normalizer with metaphone processing.
      class Normalizer
        def initialize(normalizer_to_wrap)
          @normalizer_to_wrap = normalizer_to_wrap
        end

        def add_normalization(original_sql)
          otherwise_normalized_sql = Arel.sql(
            normalizer_to_wrap.add_normalization(original_sql)
          )

          Arel::Nodes::NamedFunction.new(
            "pg_search_dmetaphone_alt",
            [otherwise_normalized_sql]
          ).to_sql
        end

        private

        attr_reader :normalizer_to_wrap
      end
    end
  end
end
